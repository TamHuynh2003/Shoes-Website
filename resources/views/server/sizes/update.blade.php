...
<div class="row">
    <div class="col-xxl">
        <div class="card mb-4">
            <div class="card-body">
                <form method="POST" action="{{ route('sizes.update', ['id' => $sizes->id]) }}">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Kích Cỡ </label>

                        <div class="input-group input-group-merge">
                            <input type="text" name="name" value="{{ $sizes->name }}" class="form-control">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="is_deleted" class="form-select">
                            ...
                            <option value="{{ $del->id }}" @if ($del->id == $sizes->is_deleted) selected
                                @endif>
                                {{ $del->name }}
                            </option>
                            @endforeach
